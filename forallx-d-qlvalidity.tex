...
\section{Part \thechapcount.\theseccount: Extensionality}\stepcounter{seccount}
...
\section{Part \thechapcount.\theseccount: Extensions}\stepcounter{seccount}
...
\section{Part \thechapcount.\theseccount: Many-Placed Predicates}\label{s.manyplaced}\stepcounter{seccount}
...
\subsection{Features of Many-Placed Predicates}\label{ss.featuresmanyplaced}
...
\subsubsection{Reflexivity}\label{sss.reflexive}
...
\section{Part \thechapcount.\theseccount: Interpretations}\stepcounter{seccount}
...
\end{center}
 ...
 \subsection{Using Features of Relational Predicates in Diagrams}
 
 While it is true that a relational predicate could have a feature under one interpretation and not have that feature in another, you should notice that, by definition, if a relational predicate as one of the features which we listed, then that feature applies in every instance of it (within that interpretation). For example, we can make a diagram for the relation Lxy with a two member domain \{0,1\}, specifying that L is reflexive like so:
 
 \begin{center}
	\begin{tikzpicture}[modal]
		\node[world] (w1)  {0};
		\node[world] (w2) [ right=of w1]{1};
...
\begin{itemize}
\item[domain:] $0,1$
\item[L:]$\openntuple 0,0\closentuple$,$\openntuple 0,1\closentuple$,$\openntuple 1,1\closentuple$  
\end{itemize}

Notice that while L is reflexive in this interpretation, it is not only reflexive, it has more relations than just something to itself. This same diagram could be used for a simple case of a relational predicate which is serial because every thing has that relation to something. The above interpretation of L, it should be clear is not symmetric. There is the relation $\openntuple 0,1\closentuple$ but not the relation $\openntuple 1,0\closentuple$. If L were symmetric in a given interpretation, then the diagram would need to have all of the arrows be double arrows, like so: 
...
\begin{itemize}
\item[domain:] $0,1$
\item[L:]$\openntuple 0,0\closentuple$,$\openntuple 0,1\closentuple$,$\openntuple 1,0\closentuple$,$\openntuple 1,1\closentuple$  
\end{itemize}

The above diagram, you might notice, is also transitive, because it only has a domain of two, is symmetric, and is reflexive. It is possible, however, to have an interpretation which is reflexive and symmetric but not transitive, like so:

 \begin{center}
	\begin{tikzpicture}[modal]
		\node[world] (w1)  {0};
		\node[world] (w2) [ right=of w1]{1};
		\node[world] (w3) [right=of w2]{2};
		\path[->] (w2) edge[reflexive above] (w2);
		\path[->] (w1) edge[reflexive above] (w1);
...
\begin{itemize}
\item[domain:] $0,1,2$
\item[L:]$\openntuple 0,0\closentuple$,$\openntuple 0,1\closentuple$,$\openntuple 1,0\closentuple$,$\openntuple 1,1\closentuple$,$\openntuple 1,2\closentuple$,$\openntuple 2,1\closentuple$,$\openntuple 2,2\closentuple$  
\end{itemize}

The reason it is not transitive is because $\openntuple 0,1\closentuple$ and $\openntuple 1,2\closentuple$ hold but $\openntuple 0,2\closentuple$ does not hold. To finish out this simple example, to make an interpretation where L is also transitive, we need to include that additional double arrow:
...
\begin{itemize}
\item[domain:] $0,1,2$
\item[L:]$\openntuple 0,0\closentuple$,$\openntuple 0,1\closentuple$,$\openntuple 0,2\closentuple$,$\openntuple 1,0\closentuple$,$\openntuple 1,1\closentuple$,$\openntuple 1,2\closentuple$,$\openntuple 2,0\closentuple$,$\openntuple 2,1\closentuple$,$\openntuple 2,2\closentuple$  
\end{itemize}
...
\end{ekey}
\section{Part \thechapcount.\theseccount: Atomic Sentences}\stepcounter{seccount}
...
\section{Part \thechapcount.\theseccount: Connectives}\stepcounter{seccount}
...
\section{Part \thechapcount.\theseccount: Quantifiers}\stepcounter{seccount}
...
\section{Part \thechapcount.\theseccount: Semantic Concepts}\stepcounter{seccount}
...
\end{ebullet}}
\section{Part \thechapcount.\theseccount: Using Interpretations}\stepcounter{seccount}
...
\section{Part \thechapcount.\theseccount: Reasoning about All Interpretations}\stepcounter{seccount}
\label{allinterp}
\subsection{Tautologies and contradictions}
